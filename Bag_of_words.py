...
def BM_25F(matrice,liste_doc_traités):
    ...
    b,k=0.75,1.2
    # print(mean_len_doc)
    """ tentative d'optimisation en se limitant au valeurs non nulle de la matrices CSR """
    (liste_i,liste_j)=matrice.nonzero()
    liste_i=list(liste_i)
    liste_j=list(liste_j)
    logging.info("Comparaison {} pour {}".format( nb_doc*long_voca,len(liste_j)))
    for compteur in range(len(liste_i)):
        i,j=liste_i[compteur],liste_j[compteur]
        if nb_doc_terme[j]!=0:
           para_a=(matrice_pond[i,j]*(k+1))/(k*((1-b)+b*(longueur_doc[i]/mean_len_doc))+matrice_pond[i,j])
           para_b=np.log((nb_doc-nb_doc_terme[j]+0.5)/(nb_doc_terme[j]))
           W[i,j]=para_a*para_b
    return W
import pickle
docment_traites=pickle.load(open("docment_traites.p",'rb'))
input_liste=[object_texte.txt_lemma_str for doc in docment_traites for (object_texte,nature,ID) in doc]
