    ...
    for item in liste_inter:
        ...
        if isinstance(item.data, class_xml.section):
            if len(item.data.Nom)>0 and item.data.Nom is not None:
                liste_finale.append((class_texte.Texte(item.data.Nom),"Section",ID))
            ...
            if len(item.data.Titre)>0 and item.data.Titre is not None:
                liste_finale.append((class_texte.Texte(item.data.Titre),"Doc",ID))
        else:
            if len(item.data.texte.strip())>0 and item.data.texte.strip() is not None:
                #print("data :"+item.data.texte)
                liste_finale.append((class_texte.Texte(item.data.texte),"Autre",ID))
    return liste_finale
...
input_liste=[object_texte.txt_lemma_str for doc in liste_docments_traités for (object_texte,nature,ID) in doc]
liste_id=[ID for doc in liste_docments_traités for (object_texte,nature,ID) in doc]
