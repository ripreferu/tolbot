...
#import stanza
#nlp=stanza.Pipeline(processors="tokenize,pos,lemma",lang="fr",logging_level="ERROR")
...
dico_doc=dict(enumerate([ DocID for doc in doc_traites for (objet_texte,nature,DocID) in doc]))

def protocole_sac_de_mot(matrice,Req, dico_doc,dico_mots):
    """ la matrice définit le modèle sac de mots basée sur une matrice terme documents:
    la réquête est lématisé sous une forme d'une liste de lemmes
    le dico_doc fait le lien numéro de ligne ->documents (forme du documents à définir)
    ...
    for ligne in range(nbre_doc):
        ...
        score.append((dico_doc[ligne],(vec_doc@vec_Req)/long_req))
        #problème sur la ligne il me faut le docID associé à la ligne
    # la liste de score donne le score de la réquête vis_a_vis du corpus
...
def wmd(doc1,doc2,matrice_plgt):
    """ Soit doc_vec2 et doc_vec1 deux doc encodées à la one_hot sur un vocabulaire connu
    ...
    if len(doc2)*len(doc1)==0:
        logger.info("l'un des 2 documents est vide --> distance infinie entre les 2 docs")
        return float('inf')
    ...
    long_voca=len(dico) # la longueur du vocabulaire s'applique ici uniquement aux 2 documents
    docset1=set(doc1)
    docset2=set(doc2)
    ...
    for i,t1 in enumerate(dico.keys()):
        ...
        for j,t2 in enumerate(dico.keys()):
            if t2 not in docset2 or distance_matrix[i,j]!=0.0:
                continue
            distance_matrix[i, j] = distance_matrix[j, i] = np.sqrt(np.sum((matrice_plgt[t1]-matrice_plgt[t2])**2))
    ...
    def nbow(doc):
        ...
        list_freq=[doc.count(i) for i in set(doc_fusion)] #fréquence des mots
        doc_len=len(doc)
...
def protocole_plongement_mot(matrice, req, dico_mot_index,liste_oh):
    ...
    """Etape 1 transformation de la réquête pour la poser un encodage one_hot """
    req_l=[]
    for lemme in req:
        try:
...
def test_models(user_entry):
    """ Pour une réquête données, la fonctiion renvoie les score des différents modèles
    Nota Bene les scores sont  des liste des couples (DocID, valeur numérique du score)
    """
    ...
    liste_oh=[(i.oh,doc_id) for doc in doc_traites for (i,nature,doc_id) in doc]
    score_plgt_mot=protocole_plongement_mot(plongement,lemme_list,dico_mot_index,liste_oh)
#    print(len(score_plgt_mot))
    score_BM25F=protocole_sac_de_mot(BM25F,lemme_list,dico_doc,dico_mot_index)
    score_tfidf=protocole_sac_de_mot(tfidf,lemme_list,dico_doc,dico_mot_index)
...
def docID2text(DocID):
    ...
    elt=tree_node.data
    logger.debug("l'élément trouvé a les caractéristiques suivantes: \
    est une branche: {} \t est une feuille: {} \t Nature exacte: {} \t niveau{} \t    \
    ".format(tree.is_branch(node_id),tree_node.is_leaf(),tree_node.tag,tree.level(node_id)))
    return elt,Document
...
remerciement_rep = ['De rien !','C\'était un plaisir !','Pas de soucis !','Aucun problème !','Ravi d\'avoir pu t\'aider !']
...
#        liste_req_user=requete_user.split()
        if 'salut' not in requete_user:
            ...
            if bool(rem):#Faux si vide
                # signal=False
                return('Tolbot: {}'.format({rem}))
            elif bool(sal): # Faux si vide
                print('Tolbot : {}'.format(sal))
            else:
                scores=list(test_models(requete_user))
                model_liste=["plongement de mots","sac de mots BM25F","sac de mot tfidf"]
                for num,each_model in enumerate(scores):
                    print("Résultat selon le modèle {}: ".format(model_liste[num]))
                   # print(each_model[0])
                    elt,doc=docID2text(each_model[0][0])
                    print(doc.Titre+" : \n",elt.texte)
        else:
            signal = False
